...
La concurrency dell'ambiente `dev` non interrompe il run attivo e puo coalescere piu pending nel piu recente. Quando un run parte, confronta il proprio SHA con l'ultimo deploy riuscito su `main`: in questo modo include anche modifiche infrastrutturali o migration appartenute a run falliti o sostituiti. Non rieseguire un vecchio run: avvia un nuovo dispatch manuale da `main` per preservare l'ordine. Il dispatch rimane disponibile anche per riallineamenti o drift che non producono modifiche repository; i tag non attivano il deploy.
