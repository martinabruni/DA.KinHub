...
- **`Kin.KinHub.Identity.Api`** — espone autenticazione, account utente e OAuth 2.0 (Authorization Code + PKCE). **Non conosce il dominio Core**: non ha alcun riferimento a `Core.Business`/`Core.Domain`/`Core.PostgreSql`, per essere riusabile come servizio identity autonomo anche in altri progetti.
- **`Kin.KinHub.App.Functions`** — Azure Functions host che espone **tutto** il dominio Core (famiglia, membri, servizi KinHub), il ricettario con assistente AI, e le liste condivise (KinList) incluso il ciclo di vita delle operazioni audio (pubblicazione e consumo della coda, tramite `[QueueTrigger]`). È l'unico servizio che possiede la "verità" sulla famiglia dell'utente (`CoreDbContext`).

Nota: un refactor precedente ha già consolidato in questo host i progetti un tempo separati `Core.Api`, `KinRecipe.Api` e `KinList.Api`; un refactor successivo ha rimosso `Kin.KinHub.KinList.AudioWorker` (worker standalone, mai effettivamente deployato in produzione — la sua unica classe viva, il consumer dei messaggi audio, è stata spostata in `KinList.Business` dietro `IAudioProcessingQueueConsumer`) e `Kin.KinHub.Migrations.Runner` (le migrazioni EF Core ora si applicano allo startup di `Identity.Api` e `App.Functions`, dietro il flag `RunMigrationsOnStartup`).
...
Un punto architetturale importante: i contesti sono **fisicamente separati anche a livello di database logico** (schemi/DbContext distinti: `core`, `identity`, `kinlist`, `kinrecipe`) ma **condividono un'unica stringa di connessione** `KinHub`. `Kin.KinHub.App.Functions` possiede direttamente `CoreDbContext` e risolve il contesto famiglia **in-process** (`CoreFamilyContextResolver` → `IFamilyOwnershipService`); non esiste più alcuna chiamata HTTP tra host per questo scopo. `Identity.Api`, all'opposto, **non accede mai** allo schema `core` — non referenzia `Core.Business`/`Core.Domain`/`Core.PostgreSql` in alcun modo, rendendolo un servizio identity indipendente e riusabile.
...
- **Adapter** — gli SDK Azure sono incapsulati dietro interfacce (`IKinListSpeechTranscriber`, `IAudioProcessingBlobStorage`, `IAudioProcessingQueuePublisher`/`IAudioProcessingQueueConsumer`).
- **Pipeline/Middleware** — middleware ASP.NET (`JwtAuthenticationMiddleware`, solo popolamento `CurrentUser` in Identity.Api) + `FunctionsAuthorizationService` (guardia esplicita di autenticazione/family-context in App.Functions, non essendoci pipeline `[Authorize]` nelle Azure Functions).
- **Background Worker** — `IdempotencyRecordCleanupService` (pulizia periodica, in App.Functions). Il consumo della coda audio avviene invece tramite `[QueueTrigger]` nativo di Azure Functions (`AudioQueueFunctions` → `IAudioProcessingQueueConsumer`), non un `BackgroundService` standalone.
...
- **Separazione dei grafi DI**: `Identity.Api` non registra nulla del dominio Core (né `AddKinHubFamilyBusiness` né `Core.PostgreSql`); tutto il dominio Core/KinList/KinRecipe vive solo in `App.Functions`. Non assumere che tutti i service siano disponibili in ogni host.
