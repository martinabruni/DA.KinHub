...
public sealed class AudioProcessingQueueConsumer : IAudioProcessingQueueConsumer
{
    ...
    {
        var payload = AudioQueueMessageSerializer.TryDeserialize(messageText);
        if (payload is null)
        {
            _logger.LogError("Invalid audio queue message payload.");
