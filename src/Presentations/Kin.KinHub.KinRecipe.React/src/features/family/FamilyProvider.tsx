...
export function FamilyProvider({ children }: { children: ReactNode }) {
  ...
  const updateNameMutation = useMutation({
    mutationFn: (name: string) => {
      ...
      return identityApiClient.patch(`/api/families/${currentFamily.id}`, { name });
    },
    onSuccess: () => {
  ...
  const addMemberMutation = useMutation({
    mutationFn: (name: string) => {
      const currentFamily = queryClient.getQueryData<Family>(["family"]);
      if (!currentFamily?.id) throw new Error("Family not loaded");
      return identityApiClient.post(`/api/families/${currentFamily.id}/members`, { name });
    },
  ...
  const updateMemberMutation= useMutation({
    mutationFn: ({ memberId, name }: { memberId: string; name: string }) => {
      const currentFamily = queryClient.getQueryData<Family>(["family"]);
      if (!currentFamily?.id) throw new Error("Family not loaded");
      return identityApiClient.put(`/api/families/${currentFamily.id}/members/${memberId}`, {
        name,
      });
    },
    onSuccess: () => invalidate(),
  ...
  const removeMemberMutation= useMutation({
    mutationFn: (memberId: string) => {
      const currentFamily = queryClient.getQueryData<Family>(["family"]);
      if (!currentFamily?.id) throw new Error("Family not loaded");
      return identityApiClient.delete(`/api/families/${currentFamily.id}/members/${memberId}`);
    },
    onSuccess: () => {
  ...
  const createFamilyMutation = useMutation({
    mutationFn: (payload: {
      familyName: string;
      ownerProfileName: string;
    }) => identityApiClient.post("/api/families", payload),
    onSuccess: () => {
      invalidate();
  ...
  const leaveFamilyMutation = useMutation({
    mutationFn: () => {
      ...
      return identityApiClient.delete(`/api/families/${currentFamily.id}/members/${activeMember.id}`);
    },
    onSuccess: () => {
  ...
  const deleteFamilyMutation = useMutation({
    mutationFn: () => {
      const currentFamily = queryClient.getQueryData<Family>(["family"]);
      if (!currentFamily?.id) throw new Error("Family not loaded");
      return identityApiClient.delete(`/api/families/${currentFamily.id}`);
    },
    onSuccess: () => {
