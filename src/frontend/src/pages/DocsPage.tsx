...
export function DocsPage() {
  ...
  return (
    <PageScaffold routeId="docs">
      {page ? <article className="markdown"><h2>{page.title}</h2><p className="lead">{page.description}</p><ReactMarkdown>{page.content}</ReactMarkdown></article> : <div className="state-card">{t("docs.missing")}</div>}
    </PageScaffold>
  );
}
