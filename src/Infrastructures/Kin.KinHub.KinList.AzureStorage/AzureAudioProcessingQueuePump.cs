...
internal sealed class AzureAudioProcessingQueuePump : IAudioProcessingQueuePump
{
    ...
    public async Task<IReadOnlyList<AudioProcessingQueueMessage>> ReceiveMessagesAsync(
        int maxMessages,
    ...
    public async Task RenewMessageVisibilityAsync(
        AudioProcessingQueueMessage message,
        TimeSpan visibilityTimeout,
        CancellationToken cancellationToken)
    {
